      function [sys,x0] = evapffs(t,x,u,flag,Ename,Fname,Sname,Vname,Lname,Cname1,Cname2,Cname3,Cname4,Cname5,Cname6)

%  EVAPFFS   Multiple effect feed-forward evaporator model
%
%            For use with FOODS-LIB models
%
%            Synopsis: S-function for multiple effect feed forward evaporator 
%            steady-state model. Determines mass and energy balances, 
%            performs design at basic and advanced transport level.
%
%            Call: evapffs(t,x,u,flag,Ename,Fname,Sname,Vname,Lname,C1name,
...
%     Evaporator operation information
      ...
      Lts(E) = UOuarray(5);         % liquid concentrate total solids (w/w)
...
%     User error message:
      if (Lts(E) <= Fts)  % Liquid concentrate total solids cannot be greater
...
%     For 1st effect:
      for J = 1:length(Fcomp) 
        if (Lctype(J,1) > 1)  
          Lcomp(J,1) = Fcomp(J)*F/L(1);
        else Lcomp(J,1) = 0; 
        end 
        if (Lctype(J,1) == 1) xh2o = J; end   
      end
%     For 2nd thru Eth effects:
      for I = 2:E
        for J = 1:length(Fcomp) 
          if (Lctype(J,1) > 1)  
...
%     Vapor  and vapor condensate composition (w/w) 
      Vcomp(:,1) = abs(F.*Fcomp - L(1).*Lcomp(:,1))./V(1); 
      for I = 2:E
...
%     If feed stream and vapor stream are not fully defined, need to return
      ...
      if (Smass > 0)
        errortxt = [10 'EVAPORATOR (',Ename,') ERROR: N-Effect Forward Feed' 10 ...  
...
%       Convergence criteria for heat transfer areas
        ...
        Udiff = 1;
        criteria = 1e-2;
        while (any(Adiff > 1e-1)) %  | (any(Nt ~= mean(Nt)))
...
%           Assumes vapor out of last effect is immediately condensed.
            Tboil(E) = tpsteam(Pin(E))+BPR(E); 
            Tstcond(E) = Tboil(E-1) - BPR(E-1);
            for I = 2:E-1 
...
%         Enthalpy of vapor streams out of each effect (kJ/kg(steam))
          for I = 1:E
            if (BPR(I) == 0) 
              ...
              Vh(I) = hsteam(Tboil(I),Pin(I),Vx(I)); 
            end  
          end
...
%           Solution of heat balance for 2 Effects
            L(1) = (-F*latcond(2)-L(E)*Vh(E)+L(E)*Lh(E))/...
                  (Lh(1)-latcond(2)-Vh(E));
%           L(1) = (-F*latcond(1)-L(E)*Vh(E))/(Lh(1)-latcond(1)-Vh(E)) 
%           L(1) = (L(2)*Lh(2) + V(2)*Vh(2) - V(1)*latcond(2))/(Lh(1)) 
          else
...
%             (A for Gauss-Jordan Elimination)
            for I = 1:(E-1)
              Lcoeff(I,I) = Lh(I) - latcond(I+1) - Vh(I+1);
            end
 
            for I = 1:(E-2)
              Lcoeff(I+1,I) = latcond(I+2);
              Lcoeff(I,I+1) = Vh(I+1) - Lh(I+1);
            end
...
%           out of last effect.
            ...
            Lsol(1,1) = -latcond(2)*F;
            Lsol(E-1,1) = L(E)*(Lh(E)-Vh(E));
...
%         Steam requirement (kg/h)
          S = (L(1)*Lh(1)-F*Fh+(F-L(1))*Vh(1))/latcond(1);
%         Steam economy
          stecon = Vtotal/S;
...
%         For 2nd thru Eth effects:
          ...
          for I = 1:E
            Lcomp(xh2o,I) = 1.0 - tsolids(Lcomp(:,I),Lctype(:,I));  % water component
          end
          Lmass = [L;Lcomp;Lctype];            
...
%               else 
%                 Lrho = (Lcomp(:,I),Lctype(:,I),Tboil(I-1))  
%                 M(I) = Nt(I)*vmin*Axin*Lrho*3600;
%                 R(I) = M(I) - L(I-1);      % recirculation flow rate 
%               end  
                if (R(I) < 0) R(I) = 0; end 
...
%     Volume hold up in tubes  
      if (dlevel == 3)
        ...
        if (Din <= 2*fthick)  
          ...
          Vhold = Ntmax*pi*((Din/2)^2-(((Din-2*fthick)/2))^2)*length;
        end 
      end 
...
%       Final microbial load 
        if (I == 1)  
          Nout(I) = mdeathk(Dvalm,DtempKm,Zvalm,Eam,Noin,Tboil(I),tmin(I));  
        else 
          Nout(I) = mdeathk(Dvalm,DtempKm,Zvalm,Eam,Nout(I-1),Tboil(I),tmin(I));  
        end  
...
%           Final quality for time when 90% of feed leaves the evaporator
      ...
      elseif (dlevel >= 4) &  all(Mme(ulocate('mass')) == Fme(ulocate('mass')))
        for I = 1:E
%         Temperature for quality assessment taken as liquid conc. wall temp (K)
          if (NRei(I) < 2100)
            Tqual(I) = Twl(I);
...
%  ****************** ECONOMIC EVALUATION VARIABLE ASSIGMENTS ***************
      ...
      if (econ >= 1)
        ECONarray = UOuarray(25:34); % economic analysis array
      end
...
%   Assignment of intermediate stream names 
    for I = 1:(E-1) 
      ...
      SNNAMES(I,1:size(LIname,2)) = LIname;
    end 
    for I = 1:E-1 
...
%   Microbial and Quality Assessment results printout
    ...
    if (dlevel == 3)
    fprintf(fid,'   Single Pass (minimum) residence time  %5.2f min \n',sum(tqmax));
    fprintf(fid,'   Minimum Process Temp taken as Internal Temperature \n');
    elseif (dlevel >= 4)
    ...
    fprintf(fid,'   Maximum Process Temp taken as Wall or Internal Temperature \n');
    end
...
%     System outputs microbial array
      SOMARRAY = [Lmdk(:,E);tmin(E);Tbulk(E)-Tref];
...
%     System outputs quality array
      SOQARRAY = [Lqual(:,E);tqmax(E);Tqual(E)-Tref];
